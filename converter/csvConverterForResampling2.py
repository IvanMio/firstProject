...
day_grouped = pd.DataFrame()
week_grouped = pd.DataFrame()
month_grouped = pd.DataFrame()
year_grouped = pd.DataFrame()
years = [2015, 2016, 2017, 2018, 2019, 2020]

for y in years:
    start_range = str(y) + '-01-01'
    end_range = str(y) + '-12-31'

    mask = (res.index >= start_range) & (res.index <= end_range)
    res_t = res.loc[mask]
    res_grouped = res_t['log'].groupby([res_t.index.hour, res_t.index.minute]).mean()
    ...
    day_grouped[str(y)] = df_res_grouped['Sum']

    mask2 = (res2.index >= start_range) & (res2.index <= end_range)
    res2_t = res2.loc[mask2]
    res2_grouped = res2_t['log'].groupby([res2_t.index.dayofweek, res2_t.index.hour, res2_t.index.minute]).mean().reindex(index_list1)
    ...
    df_res2_grouped['Datetime'] = dt.datetime(2021, 6, 14) + df_res2_grouped['Timedelta']
    df_res2_grouped.set_index('Datetime', inplace=True)
    # print(df_res2_grouped.head(60))
    # print(df_res2_grouped.tail(60))
    week_grouped[str(y)] = df_res2_grouped['Sum']

    mask3 = (res3.index >= start_range) & (res3.index <= end_range)
    res3_t = res3.loc[mask3]
    res3_grouped = res3_t['log'].groupby([res3_t.index.day, res3_t.index.hour, res3_t.index.minute]).mean()
    ...
    month_grouped[str(y)] = df_res3_grouped['Sum']

    mask4 = (res4.index >= start_range) & (res4.index <= end_range)
    res4_t = res4.loc[mask4]
    res4_grouped = res4_t['log'].groupby([res4_t.index.month, res4_t.index.day, res4_t.index.hour]).mean()
    ...
    print('str(y)', str(y))
    year_grouped[str(y)] = df_res4_grouped['Sum']
    print(year_grouped.head(60))
    print(year_grouped.tail(60))


path_a = 'C:/Users/ivanm/Documents/Currency/AUDNZD/analysis/'
day_grouped.to_excel(path_a + 'day_grouped.xlsx')
week_grouped.to_excel(path_a + 'week_grouped.xlsx')
month_grouped.to_excel(path_a + 'month_grouped.xlsx')
year_grouped.to_excel(path_a + 'year_grouped.xlsx')
...
mpl.xlim(['2024-01-01', '2024-12-31'])
mpl.show()
