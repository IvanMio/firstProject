...
def strategy(symbol_in, volume_in, ticks_to_target_in, ticks_to_stop_in,
    ...
    dty = np.dtype([('Entry_Date_Time', 'S26'), ('Entry_Price', '<f8'), ('Exit_Date_Time', 'S26'),
                    ('Exit_Reason', 'S7'), ('Exit_Price', '<f8'), ('P_L_100t', '<f8'), ('Entry_hour', '<f8'),
                    ('Ticks_to_target', '<f8'), ('Ticks_to_stop', '<f8')])
    new_rows = np.empty(delta.days + 1, dty)
    k = 0
...
#        finally:
        ...
        # print(date_slice)
        new_rows[k] = (str(entry_d_und_t), entry_price,  str(exit_d_und_t), exit_reason, exit_price, p_und_l_100t,
                       entry_hour,  ticks_to_target, ticks_to_stop)
